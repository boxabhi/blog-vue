      ...
      <div class="row">
        <div class="col-lg-8">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="post-entry-2 d-flex bg-light" v-for="blog in latest_blog.slice(0,4)"
               v-bind:key="blog.id">
        ...
        <div class="col-lg-4">
          ...
          <div v-for="(trend,times) in trending.slice(0,4)" v-bind:key="trend.id">
            <div class="trend-entry d-flex">
              <div class="number align-self-start justify-content">
                {{times +1}}

              </div>
              <div class="trend-contents">
                <h2 class="justify-content">
                 <router-link :to="`/detail/${trend.id}`">
                  <a>
                      {{ (trend.title)}}
                    </a></router-link>
                </h2>
                <div class="post-meta">
                  <span class="d-block"><a href="#">{{ (trend.author)}}</a> in 
                  <a href="#">{{ (trend.category)}}</a></span>
                  <span class="date-read">Jun 14 <span class="mx-1">&bullet;</span> 3 min read <span
                      class="icon-star2"></span></span>
                </div>
              </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      async fetchTrending() {
        var proxy = "https://cors-anywhere.herokuapp.com/";
        var url = "http://ckclub.in/v2/api/trending";
        var final_url = proxy + url;
        await fetch(final_url)
